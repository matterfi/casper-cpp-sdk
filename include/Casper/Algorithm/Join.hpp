...
template <size_t N> struct string_size_impl<const char[N]>
{
...
template <size_t N> struct string_size_impl<char[N]>
{
    static constexpr size_t size(char (&s)[N])
    {
        return N ? strlen(s) : 0;
    }
};
...
template <> struct string_size_impl<std::string_view>
{
    static size_t size(std::string_view s)
    {
        return s.size();
    }
};
...
template <typename String> struct concatenate_impl<String>
{
    ...
    static void concatenate(std::string& result, String&& s)
    {
        result += s;
    }
};
...
template <typename... Strings> std::string join(Strings&&... strings)
{
    std::string result;
